...
The agent's default reading order is L3 → L1 (query) → L2 (read top-k dossiers) → L4 (only when it needs source-level detail). Humans can read every layer directly — that is a feature, not a side effect: the KB is independently valuable to the estimating group even with no agent attached.
