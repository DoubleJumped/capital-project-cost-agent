...
and its successors (ColQwen2/ColQwen3) [15] take a different route: a vision-language model produces multi-vector
...
12. [Large Language Models with Human-In-The-Loop Validation for Systematic Review Data Extraction (ResearchGate)](https://www.researchgate.net/publication/388316685_Large_Language_Models_with_Human-In-The-Loop_Validation_for_Systematic_Review_Data_Extraction) plus search-synthesized confidence-routing patterns — uncertainty sampling, self-consistency confidence scoring, two-tier multi-model→expert routing. Note: the **~87% human-confirms-LLM** figure is from this single *medical* systematic-review study and is used in-text only as an illustrative analogue, not a design constant for gas-utility cost extraction.
13. [RAG vs GraphRAG — Cognilium](https://cognilium.ai/blogs/rag-vs-graphrag) — practitioner writeup; source of the *"a two-week reranking project saved them a six-month migration"* line (previously misattributed to [1]).
14. [Introducing Contextual Retrieval — Anthropic](https://www.anthropic.com/engineering/contextual-retrieval) — rigorously measured: contextual embeddings + contextual BM25 cut top-20 retrieval failures by **49%**, **67%** with reranking added (contextual embeddings alone: 35%). The trustworthy anchor for the hybrid+rerank gain claim.
15. [ColPali: Efficient Document Retrieval with Vision Language Models (arXiv 2407.01449)](https://arxiv.org/abs/2407.01449) — multi-vector visual late-interaction retrieval over page *images* (no OCR), successors ColQwen2/ColQwen3; outperforms parse-then-embed pipelines on the ViDoRe benchmark. Relevant as a retrieval alternative/complement for the scan- and drawing-heavy legacy corpus.
16. [PARSE: LLM-Driven Schema Optimization for Reliable Entity Extraction (arXiv 2510.08623)](https://arxiv.org/abs/2510.08623) — ARCHITECT (schema auto-refinement) + **SCOPE** (reflection-based extraction with combined static + LLM guardrails, reported **92% extraction-error reduction within the first retry**); SCOPE targets the messy nullable-field problem central to this project.
