...
- **Never ship a single number.** Every estimate is a distribution. The industry standard our estimators already trust — AACE estimate classes — is defined *in ranges* (Class 5: −50%/+100%; Class 4: −30%/+50%; Class 3: −20%/+30%) and those ranges are asymmetric (overrun risk exceeds underrun) [1]. Our output must speak this language.
...
Capital-cost estimation is irreducibly uncertain, and the profession has codified this. The **AACE International** estimate-classification system ties expected accuracy to the *maturity of project definition*, not to effort spent: a Class 5 concept-screening estimate carries a −50%/+100% range, Class 4 −30%/+50%, Class 3 (budget authorization) −20%/+30%, tightening to −10%/+15% at Class 1 [1]. Two features matter for us. First, ranges are **asymmetric** — the upside (overrun) tail is fatter, reflecting real capital-project behavior. Second, AACE is explicit that these published ranges are *guidelines only*; the actual range must be established by project-specific quantitative risk analysis (typically Monte Carlo), not applied by lookup table [1].
...
| **Jackknife+** | *n* leave-one-out models; interval from LOO residual quantiles applied to LOO predictions | **1−2α** (proven) | Best data efficiency; can be unstable when *n* ≈ #features [3] |
...
- **Better signals exist but are indirect.** **Self-consistency / semantic entropy**: sample the model multiple times and measure disagreement, clustering by semantic equivalence rather than surface strings [11]. Predictive/semantic entropy over sampled answers is a stronger uncertainty signal than any single verbalized number, and self-consistency sampling itself lifts accuracy on numeric-reasoning tasks (+27.6pp on GSM8K, +23.7pp on MATH) [11].
...
This mirrors a live concern in the LLM-eval literature: benchmark contamination audits find leakage of 1–45% across popular QA benchmarks, and "search-time contamination" inflates deep-research-agent scores when the agent can retrieve the answer [13]. Our corpus is private, which removes *training* contamination — but *retrieval* leakage (the agent pulling the held-out project's own final cost report) is the same failure and must be engineered out per fold.
...
As projects complete, the corpus grows and the cost regime drifts — exchangeability (the assumption behind vanilla conformal guarantees) breaks. This is a solved-ish problem: **Adaptive Conformal Inference (ACI)** (Gibbs & Candès, 2021) treats the miscoverage level as an online-learned parameter, tightening or widening the band based on recent under/over-coverage feedback, with extensions (Conformal PID, bias-corrected ACI) for stronger tracking under shift [5]. For us this means: escalation shocks (post-COVID input-cost spikes) show up as rising backtest error on recent-era slices → ACI-style feedback re-widens ranges automatically while the underlying escalation indices are re-examined. Recalibration is a versioned artifact tied to the KB commit hash; the learning loop is **reviewed pipeline events, not silent runtime memory writes** (`plan/05` §4).
