...
- **Normalization is non-negotiable and must be explicit metadata.** Every historical cost must be normalized to a common basis before comparison: time (escalation via StatCan BCPI — which now covers **15 CMAs including the prairie cities Regina, Saskatoon, Calgary, Edmonton and Winnipeg** — the utility-specific **Handy-Whitman gas index**, or ENR CCI as a shape cross-check), location factor, scope/capacity, and currency. Normalization runs both ways: restate historicals *back* to a common date, then escalate the estimate *forward* to the construction spend window (out-turn / as-spent dollars). Store both the raw and normalized figure plus the index/factor used, so estimates are auditable. [4][6][11][23][24]
...
The evidence that conventional estimating fails is overwhelming. In Flyvbjerg's database of **16,000+ projects, only ~0.5% hit budget, schedule, and benefits**; only ~47.9% meet budget or better; only ~8.5% meet budget *and* schedule. [8] Overruns are **fat-tailed** — mean overruns and, crucially, the size of the upper tail vary sharply by type. Here the **"% in fat tail" column is the share of projects in that class exceeding a +50% cost overrun** (the threshold Flyvbjerg uses to demarcate the tail), and the last column is the mean overrun *among* those tail projects: [9]
...
Escalation is **two-directional**. First restate every historical cost *back* to a common reference date. Then — the step that turns a normalized history into a *new-project* estimate — escalate *forward* to the project's construction spend window, producing out-turn / as-spent (then-year) dollars applied across the cash-flow curve rather than a single base-year figure. The OSD CAPE *Inflation & Escalation Best Practices Handbook* is the clearest public treatment of this back-to-base vs forward-to-outlay distinction and of spreading escalation over an outlay profile. [24] Candidate indices:
- **Statistics Canada Building Construction Price Index (BCPI):** quarterly, base 2017=100 (2015 for non-residential/high-rise), now covering **15 CMAs — including the prairie cities Regina, Saskatoon, Calgary, Edmonton and Winnipeg** (Regina was among four CMAs added with data from 2023 onward). Explicitly intended for "making adjustments to project costs for escalation," and the **only prairie-anchored series** here. Best default for Canadian building-type work. [11][25]
- **Handy-Whitman Index of Public Utility Construction Costs:** the utility-industry standard, published semi-annually by Whitman, Requardt & Associates with a **dedicated gas utility-plant index** (separate electric/gas/water series) by North-American region. Regulators, utilities and valuation engineers use it to trend original-cost records — making it far more apt for gas-station/pipeline plant than building-oriented BCPI or ENR CCI. **Caveat:** its regions are US-centric with no native prairie point, so it serves as an escalation-*shape* cross-check, not a prairie anchor. [23]
- **ENR Construction Cost Index (CCI) / Building Cost Index (BCI):** a labor+materials weighted index published monthly; covers 20 US cities plus **Montreal and Toronto** only in Canada — again no native prairie point, so cross-check / US-sourced-equipment use only. [4][6]
- **Gap flag (largely resolved):** for linear/oil-and-gas infrastructure the relevant escalation baskets are S&P Global's (formerly IHS/CERA) **UCCI — Upstream Capital Costs Index**, a 28-project global basket that explicitly *includes onshore, offshore, pipeline and LNG* projects, and its downstream counterpart **DCCI** (30+ refining/petrochemical projects). These are proprietary/paid and global rather than prairie-specific, so a **prairie-calibrated pipeline $/inch-mile CER mined from our own corpus** remains the real target — but Handy-Whitman's gas index now fills most of the utility-plant escalation gap the earlier draft flagged.
...
- **Compressor stations** are commonly modeled parametrically on **horsepower**, with pronounced **economies of scale**: the study behind OGJ's regression models found a 2,000 hp station costs **~3.2× per-hp** what a 30,000 hp station does, and unit total cost for a 5,000 hp station ranged ~$2,097/hp (Central US) to ~$2,820/hp (Western US) — regional spread ~34%. Model form: nonlinear regression on capacity + capacity² with regional dummies. **Vintage caveat:** this regression is built on 220 stations **completed 1992–2008**, so — exactly like the dated 2013 $1,500/hp anchor below — the absolute $/hp levels are badly stale and need heavy forward escalation before any use. **Take the 3.2× economies-of-scale factor and the ~34% regional spread as shape/ratio evidence; do not treat the dollar figures as usable rates.** [18]
...
The agent design aligns with active research: NLP ensembles aligning quantity take-offs to cost indexes (2025), RAG + fine-tuning outperforming baseline GPT-4 for structured construction risk/knowledge tasks, and LLMs shown to beat classical ML baselines for early-stage estimation *especially because they tolerate missing features* — directly relevant to our heterogeneous, gap-ridden corpus. [21][22]

**Treat the central architectural claim as a hypothesis, not a settled finding.** The stronger assertion — that a *curated, LLM-legible knowledge base* (the Karpathy "wiki the agent reads" idea) beats naive RAG-over-everything *for cost estimation specifically* — rests here on only two adjacent citations and general RAG-vs-baseline evidence, none of it a head-to-head test on this task. So this is best stated as **the core design hypothesis this project is built to test**, not an established result. The evaluation plan should measure it directly: curated-KB agent vs a raw-RAG baseline over the same 200-project corpus, scored on estimate accuracy, comparable-selection quality, and citation faithfulness. [21][22]
...
3. **Persist normalization as metadata on every cost.** Store {raw cost, currency, cost date, index used + values, location factor, capacity basis, normalized cost, cost-basis flags}. *Pro:* auditable, defensible, re-runnable when indices update. *Con:* upfront ingestion effort and requires choosing indices — default to StatCan **BCPI**, now the only prairie-anchored series (confirmed Regina *and* Saskatoon coverage), use the utility-specific **Handy-Whitman gas index** for gas-plant work, and treat both Handy-Whitman (US/Canada regions) and **ENR CCI** (20 US cities + Montréal/Toronto) as escalation-*shape* cross-checks since neither has a native prairie point. Persist escalation in **both directions** (back to base, then forward to the spend window). Longer-run, mine a prairie **$/inch-mile** CER from our own corpus in place of the global, paid UCCI/DCCI baskets.
...
1. [AACE International — PGD-01, Guide to Cost Estimate Classification Systems](https://library.aacei.org/pgd01/pgd01.shtml) — AACE's own authoritative roadmap to the Class 1–5 system: maturity of project definition as the sole primary characteristic, asymmetric accuracy ranges as guidelines requiring project-specific risk analysis. Public authoritative substitute for the paywalled 17R-97/18R-97.
...
11. [Statistics Canada — Technical Guide for the Building Construction Price Index](https://www150.statcan.gc.ca/n1/pub/62f0014m/62f0014m2022005-eng.htm) — BCPI base 2017=100, used for cost escalation. (Coverage now 15 CMAs — see [25].)
...
23. [Handy-Whitman Index of Public Utility Construction Costs — Whitman, Requardt & Associates](https://wrallp.com/about-us/handy-whitman-index) — utility-industry-standard index published semi-annually with separate electric/**gas**/water series by North-American region; used by regulators/utilities/valuation engineers to trend original-cost records. US-centric regions (no native prairie point).
24. [OSD CAPE — Inflation and Escalation Best Practices for Cost Analysis: Analyst Handbook (2021)](https://www.cape.osd.mil/files/otherGuides/OSDCAPEEscalationHandbook2021.pdf) — authoritative treatment of base-year vs then-year/as-spent (out-turn) dollars and escalating forward across an outlay/spend profile; grounds the forward-escalation step.
25. [Statistics Canada — The Daily, Building Construction Price Index update (April 2026)](https://www150.statcan.gc.ca/n1/daily-quotidien/260428/dq260428b-eng.htm) — BCPI now covers **15 CMAs including Regina** (added with data from 2023 onward), alongside Saskatoon, Calgary, Edmonton and Winnipeg.

*Verification flags: the utility-plant escalation gap is now largely filled by the Handy-Whitman gas index [23]; the oil-and-gas capital-cost baskets UCCI/DCCI [S&P Global] cover pipeline/LNG but are global and paid — a prairie-calibrated $/inch-mile CER mined from our own corpus remains the target. The OGJ $/hp figures (1992–2008) and the $1,500/hp and $25M/station figures (2013) are dated US anchors, usable as shape/ratio evidence only, not prairie-calibrated rates. The primary 18R-97/17R-97 documents are paywalled; AACE's public PGD-01 [1] is the authoritative substitute for the class table.*
